...
namespace WPFSerialAssistant
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public void Save2Database(string s){
            string ss = "%(1)(.3)%";
            string[] spli=s.Split(new char[]{'%','%'});
            for (int i = 0; i < spli.Length; i++)
            {
                //recvDataRichTextBox.AppendText(i+":"+spli[i]);
            }
            if (spli.Length >= 3)
            {
                string[] spli2 = spli[1].Split(new char[] { '(', ')' });
                recvDataRichTextBox.AppendText("\n编号：" + spli2[1]+" 用户，将要消费："+spli2[3]+"元；\n");
                //recvDataRichTextBox.AppendText("\n" + spli2[3]);
                string ConnectionString = "server=.;database=fingerprintUser;Integrated Security=SSPI;";
                SqlConnection conn = new SqlConnection(ConnectionString);
                conn.Open();
                string sql = "select *from users where fingerId=" + Convert.ToInt32(spli2[1]);
                ...
                {
                    double a = Convert.ToDouble(ds.Tables[0].Rows[0]["money"]);
                    a = a - Convert.ToDouble(spli2[3]);
                    ds.Tables[0].Rows[0]["money"] = a;
                    //da.Update(ds, "users");
                    sql = "update users set money=" + (object)a + " where fingerId=" + (object)Convert.ToInt32(spli2[1]);

                    //string MyConn = "server=127.0.0.1;uid=users;pwd=123456;database=Northwind;Trusted_Connection=no";
                    ...
                    {
                        recvDataRichTextBox.AppendText(ex + "Exception caught.");
                    }
                    //da = new SqlDataAdapter(sql, conn);
                    MyConnection.Open();
                    sql = "select  money from users where fingerId=" + Convert.ToInt32(spli2[1]);
                    da = new SqlDataAdapter(sql, conn);
                    da.Fill(ds);
                    double b = Convert.ToDouble(ds.Tables[0].Rows[0]["money"]);
                    string result ="@"+spli2[1] + "," + b + "%";
                    recvDataRichTextBox.AppendText(result);
                    //recvDataRichTextBox.AppendText("\n" + spli2[3]);
                    SerialPortWrite(result, true);
                    int u;
                    MyConnection.Close();
                    //da.Fill(datagrrid1);
                }
        ...
        {
            //string sql = "insert into users(id,fingerId,name,money)values(10,10,10,10)";
            //string ConnectionString = "server=.;database=fingerprintUser;Integrated Security=SSPI;";
            ...
            //for (int j = 0; j < 300; j++)
            //{
            //    string MyUpdate = "insert into users(id,fingerId,name,money)values(" + j + "," + j + "," + j + ",200)";
            //    SqlCommand MyCommand = new SqlCommand(MyUpdate, MyConnection);
            //    try
            //    {
            //        MyConnection.Open();
        ...
        private void ReceivedDataHandler(object obj)
        {
            ...
            this.Dispatcher.Invoke(new Action(() =>
            {
                if (showReceiveData)
                {
                    ...
                    String stri = Utilities.BytesToText(recvBuffer, receiveMode, serialPort.Encoding);
                    //recvDataRichTextBox.AppendText(stri);
                    Save2Database(stri);
